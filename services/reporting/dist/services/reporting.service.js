...
class ReportingService {
    ...
    async getTransactions(filters = {}) {
        const cacheKey = redis_service_1.RedisService.generateTransactionKey(filters);
    ...
    async getDailyBalance(date) {
        const cacheKey = redis_service_1.RedisService.generateBalanceKey(date);
        // Try to get from cache
        const cached = await this.redis.get(cacheKey);
        if (cached) {
     ...
     */
    async getBalanceHistory(filters) {
        const cacheKey = redis_service_1.RedisService.generateBalanceHistoryKey(filters.startDate, filters.endDate);
        // Try to get from cache
        const cached = await this.redis.get(cacheKey);
        if (cached) {
     ...
     */
    async getDashboard() {
        const cacheKey = redis_service_1.RedisService.generateDashboardKey();
        // Try to get from cache
        const cached = await this.redis.get(cacheKey);
        if (cached) {
     ...
     */
    async getCategorySummary(month) {
        const cacheKey = redis_service_1.RedisService.generateCategorySummaryKey(month);
        // Try to get from cache
        const cached = await this.redis.get(cacheKey);
        if (cached) {
