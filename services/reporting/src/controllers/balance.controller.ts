...
export class BalanceController {
  ...
  constructor(reportingService: ReportingService) {
    ...
    this.logger = new Logger({ service: 'BalanceController' });
  }
   ...
   */
  async getDailyBalance(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  async getBalanceChart(req: Request, res: Response): Promise<void> {
    try {
      const { startDate, endDate } = req.query;
      ...
      const chartData = result.history.map((day: any) => ({
        date: day.date,
